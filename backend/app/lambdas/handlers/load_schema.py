...
async def _handle(event: dict[str, Any]) -> dict[str, Any]:
    ...
    async def _run(session):
        ...
        # load_children=True: the default query defers schema_snapshot (it's
        # the shared ownership/status-check path most routes use without
        # reading that column) — reading it below without this is a
        # lazy-load outside the async context, MissingGreenlet in a Lambda.
        # Same fix as discover_schema.py; see docs/backendfix.md 2026-08-11c.
