-- =====================================================================
-- IRS SOI Individual Income Tax ZIP Code Data (Tax Year 2022)
-- Source: https://www.irs.gov/statistics/soi-tax-stats-individual-income-tax-statistics-2022-zip-code-data-soi
-- Real, public IRS data. 165 columns, 166,131 rows, ~216 MB CSV.
-- One row per (state, ZIP code, income bracket) with ~160 tax return
-- count/amount fields (returns filed, AGI, deductions, credits, etc).
--
-- HOW TO USE:
--   1. Open your cluster in CockroachDB Cloud Console -> SQL Shell.
--   2. Paste and run the CREATE TABLE statement below.
--   3. Paste and run the IMPORT INTO statement below. It uses WITH DETACHED
--      because CockroachDB Cloud's web SQL Shell runs each query inside an
--      explicit transaction, and IMPORT INTO refuses to run inside one
--      unless detached -- this returns a job_id immediately instead of
--      blocking the shell.
--   4. Track progress with:
--        SHOW JOBS WHEN COMPLETE (SELECT job_id FROM [SHOW JOBS]
--          WHERE job_type = 'IMPORT' ORDER BY created DESC LIMIT 1);
--      or just poll: SHOW JOBS WHERE job_type = 'IMPORT' ORDER BY created DESC LIMIT 1;
--   5. Once status = 'succeeded', verify with: SELECT count(*) FROM irs_zip_income;
-- =====================================================================
