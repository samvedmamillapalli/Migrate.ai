...
function MemoryCard({ item }: { item: MemoryListItem }) {
  ...
  return (
    ...
    >
      ...
      {!item.has_embedding ? (
        <p className="text-muted-foreground text-xs">Indexing for search…</p>
      ) : null}
...
/**
 * The full memory browser: corpus health, semantic search, and the paginated
 * list. Embedded inline inside Settings (see Block 9/10 of
 * docs/AUG18_FINAL_PUSH_PLAN.md) — previously its own standalone
 * `/dashboard/memory` route/nav item, retired once this had a home inside
 * Settings. Kept as its own component (not inlined into the Settings page)
 * so the implementation stays reusable if a second surface ever needs it.
 */
    ...
    <div className="w-full space-y-5">
      {loudWarning ? (
        <Panel
          ...
          className="flex w-full flex-col gap-2 border-[var(--tone-fail-border)] bg-[var(--tone-fail-bg)] px-6 py-5"
        >
